...
impl Grid {
    ...
    /// ConPTY anchors a shortened viewport at its cursor and repaints only the retained rows. Keep the displaced primary-screen rows in scrollback before the repaint, as Ghostty's resize does for zmx; truncating them loses early command output permanently.
    pub fn set_size(&mut self, size: Size) {
        if self.scrollback_len > 0 && size.rows < self.size.rows {
            let displaced = self.pos.row.saturating_add(1).saturating_sub(size.rows);
            for row in self.rows.drain(..usize::from(displaced)) {
            ...
            if self.scrollback_offset > 0 {
                self.scrollback_offset = self
                    .scrollback
                    .len()
                    .min(self.scrollback_offset + usize::from(displaced));
            }
            self.pos.row = self.pos.row.saturating_sub(displaced);
            self.saved_pos.row = self.saved_pos.row.saturating_sub(displaced);
        }
